...
class FountainSequence(RGBDSequence):
    def __init__(self, path: str, start_frame: int = 0, end_frame: int = -1, first_tq: list = None, load_gt: bool = False):
        super().__init__()
        self.path = Path(path)
            
        with open(self.path/"depth_vga.match", 'r') as f:
            lines = f.readlines()
            pairs = [l.strip().split(' ') for l in lines]
        self.color_names = []
        self.depth_names = []
        for pair in pairs:
            self.depth_names.append(pair[0])
            self.color_names.append(pair[-1])
        ...
        self.calib = [525., 525.0, 319.50, 239.50, 1000.0]
        ...
        if load_gt:
            gt_traj_path = list(self.path.glob("fountain_key.log"))[0]
