...
class AugICLRGBDDataset:
    def __init__(self, path, start_frame: int = 0, end_frame: int = -1, first_tq: list = None, load_gt: bool = False, register=False, mesh_gt: str = None):
        ...
        if not register:
            ...
            depth_ids, depth_timestamps = self.listdir(path, 'depth_w_noise')
        else:
        ...
        if load_gt:
            ...
            scene = path.split('/')[-1]
            gt_traj_path = os.path.join(path,scene+'-traj.txt')
