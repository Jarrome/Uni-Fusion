...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if '.png' in depth_path:
            ...
            depth_data = cv2.imread(depth_path,-1) 
...
class ScanNet_for_preprocessed(BaseDataset):
    '''
        here we use the processed frame which is every 100 frame
    '''
    def __init__(self, cfg, args, scale, device='cuda:0'
                 ):
        super(ScanNet, self).__init__(cfg, args, scale, device)
        #self.input_folder = os.path.join(self.input_folder, 'exported')
